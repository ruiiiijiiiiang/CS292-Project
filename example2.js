...
function init(tmp, treeid){
    ...
    var st = new $jit.ST({
        ...
        onBeforePlotNode: function(node){
          ...
          //option 1: display depending on ratings
          color = node.getData('color');
          node.setData('color', color);
          //option 2: if read before, yellow; else, default color
          /**
            if (node.selected) {
                node.data.$color = "#ff7";
            }
    ...
    st.onClick(st.root);
    //call lightbox of st.root's info
    //end
}
